...
<html>
	<head>
		...
		<?php if ($image && strlen($image)>1) { ?>
			<meta property="og:image" content="<?php print $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].$image; ?>" />
		<?php } ?>
		<?php if ($url_path && strlen($url_path)>1) { ?>
			<meta property="og:url" content="<?php print $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].$url_path; ?>" />
		<?php } ?><?php print $head; ?>
